...
namespace GadgetCMS.ViewComponents
{
    public class ArticleFeaturedViewComponent : ViewComponent
    {
        ...
        {
            articlesList = _context.Article.Include(c => c.ArticlePictures).Where(d => d.Featured == true).Where(e => e.ArticleVisible == true).OrderByDescending(c => c.ArticleCreated).Take(5).ToList();
